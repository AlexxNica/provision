...
func (h *DhcpHandler) buildOptions(
	...
	for c, v := range options {
		...
		log.Debugf("Received option: %v: %v", c, srcOpts[int(c)])
	}
...
func (h *DhcpHandler) ServeDHCP(p dhcp.Packet,
	...
	l.Infof("Received DHCP packet: type %s %s ciaddr %s yiaddr %s giaddr %s server %s chaddr %s on %s",
		msgType.String(),
		xid(p),
		p.CIAddr(),
		p.YIAddr(),
		...
		p.CHAddr().String(),
		cm.Dst.String())
	l.Debugf("Packet recieved on interface %s, subnet candidates are %v", h.intf(cm).Name, h.listenIPs(cm))
